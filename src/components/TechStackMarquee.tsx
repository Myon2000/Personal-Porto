...
export default function TechStackMarquee() {
  ...
  return (
    <section
      id="stack"
      className="py-20 md:py-28 border-b border-slate-200/80 dark:border-slate-800/80 bg-white dark:bg-[#090d16] transition-colors overflow-hidden"
    >
      <div className="max-w-6xl mx-auto px-4 sm:px-6 space-y-12">
        ...
        <div className="flex flex-col sm:flex-row sm:items-baseline justify-between gap-2 pb-6 border-b border-slate-200 dark:border-slate-800">
          <div className="flex items-center gap-4 font-mono text-xs text-slate-400">
            <span className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-white">05</span>
      ...
      <div className="relative mt-12 w-full overflow-hidden space-y-4">
        ...
        <div className="absolute top-0 bottom-0 left-0 w-16 sm:w-32 bg-gradient-to-r from-white dark:from-[#090d16] to-transparent z-10 pointer-events-none" />
        <div className="absolute top-0 bottom-0 right-0 w-16 sm:w-32 bg-gradient-to-l from-white dark:from-[#090d16] to-transparent z-10 pointer-events-none" />
        ...
        <div className="animate-marquee-ltr gap-3 py-1">
          {[...ROW_1, ...ROW_1, ...ROW_1, ...ROW_1].map((item, idx) => (
            <div
              key={`r1-${idx}`}
              className="inline-flex items-center gap-3 px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50/70 dark:bg-slate-900/60 hover:border-slate-400 dark:hover:border-slate-600 transition-colors shadow-2xs shrink-0 select-none"
            >
              <span
              ...
              <div className="flex items-baseline gap-2">
                <span className="font-mono text-xs font-bold tracking-wider text-slate-900 dark:text-white uppercase">
                  {item.name}
                </span>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 pt-6 flex flex-wrap items-center justify-between gap-4 text-xs font-mono text-slate-500 dark:text-slate-400">
