    ...
    >
      <div className="max-w-6xl mx-auto px-6 sm:px-10 lg:px-16 space-y-14">
        ...
        <div className="space-y-4 max-w-4xl">
          <div className="inline-flex items-center gap-2 px-2.5 py-1 rounded text-[11px] font-mono bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-slate-200">
            <span>{spotlightProject.badge[language]}</span>
          </div>
          <h2 className="text-3xl sm:text-5xl lg:text-6xl font-extrabold tracking-tight text-slate-950 dark:text-white leading-[1.05]">
            {spotlightProject.title}
          </h2>
          <p className="text-base sm:text-lg text-slate-600 dark:text-slate-400 leading-relaxed font-normal pt-2">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start pt-2">
          ...
          <div className="lg:col-span-7 space-y-4">
            ...
            <ul className="space-y-2.5 font-mono text-xs">
              {spotlightProject.highlights[language].map((item, idx) => (
                <li
                  key={idx}
                  className="flex items-start gap-3 p-3.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/40 text-slate-700 dark:text-slate-300"
                >
            ...
            <div className="flex flex-wrap gap-1.5 pt-2">
              {spotlightProject.tags.map((tag) => (
                <span
                  key={tag}
                  className="px-2.5 py-1 rounded text-[11px] font-mono bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700"
                >
                  {tag}
                </span>
              ))}
          ...
          <div className="lg:col-span-5 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 bg-slate-50/70 dark:bg-slate-900/50 space-y-5">
            ...
            <a
              ...
              className="inline-flex items-center justify-center gap-2 w-full px-5 py-3 rounded-lg bg-slate-900 hover:bg-slate-800 dark:bg-white dark:hover:bg-slate-100 text-white dark:text-slate-900 font-semibold text-xs font-mono tracking-wider uppercase transition-colors"
            >
