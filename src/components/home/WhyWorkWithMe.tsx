    ...
    >
      <div className="max-w-6xl mx-auto px-6 sm:px-10 lg:px-16 space-y-14">
        ...
        <div className="max-w-4xl space-y-4">
          <h2 className="text-3xl sm:text-5xl font-extrabold text-slate-950 dark:text-white tracking-tight leading-[1.08]">
            {language === "id"
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-2">
          {principles.map((item) => (
            <div
              key={item.letter}
              className="p-6 sm:p-7 rounded-2xl border border-slate-200 dark:border-slate-800 bg-slate-50/60 dark:bg-slate-900/40 hover:border-slate-400 dark:hover:border-slate-600 transition-colors space-y-4 flex flex-col justify-between"
            >
              <div className="space-y-3">
                <span className="w-8 h-8 rounded-full border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white font-mono text-xs font-bold flex items-center justify-center">
                  {item.letter}
                </span>
